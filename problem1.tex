...
At the most basic level, learning and detecting interaction categories requires creating two things: 1) a computational representation of an interaction; and 2) a recipe for measuring the similarity (or distance) between any two interactions. These two things together must have four attributes. First, they must be discriminative so that the similarity between instances from distinct categories is low while that for instances from the same category is high. Second, they must be insensitive to distractors, such as changes in viewpoint, participant identities, the temporal extent of the interaction, and the rate of progression within the interaction. Third, they must be robust and efficient so that large image collections and long videos of large gatherings can be processed automatically and efficiently. Fourth and finally, they must be general in the sense of being able to automatically exploit the varying levels of interaction information available in different input types, from low-resolution surveillance video to high-resolution to video at a meeting environment.

These requirements have disabled the attempt to resort to traditional group activity representations such as multi-thread HMMs that require hierarchical fine-scale explicit descriptions of atomic activities against a hand-made dictionary, coupling among them, and parameterized dynamics, and require prohibitive amount of labeled training samples.\todd{To do: explained this point}
