...
Images and videos of people reveal information about their interactions, roles, and relationships through a variety cues. The still image in the top-left of Fig.~\ref{fig:intro} suggests the two individuals are friends instead of only colleagues because of the proximity of the two detected individuals, their poses and expressions, and the scene category (restaurant/bar) in which they appear. All of these cues are accessible (with noise) using existing computer vision tools for for face detection, pose and expression estimation, and scene recognition; and their utility for social analysis has been demonstrated by recent efforts that have successfully used them to recover information about age and gender~ \cite{Gallagher}, relationship categories (e.g., father-daughter vs. siblings)~\cite{Wang2010} and event categories (e.g., beach party vs.~country bar)~\cite{Murillo2012}.

Videos contain even more social information because they provide access to more precise descriptions of social encounters. Sometimes this might be accomplished by carefully choosing one keyframe ((a) and (c) in Fig.~\ref{fig:socialbehavior}) that allows clear categorization of the interaction, for example, as a kick or punch (a) or three-way conversation in (c). Other times much more could be revealed by the spatial and temporal structure of approximately-tracked actors, as suggested by Figs.~\ref{fig:socialbehavior}(b--d), where one might observe attention response or turn-taking in a meeting or conversation; a chance encounter between two friends; or an altercation between two strangers.
...
\end{center}
\vspace{-0.25in} \caption{\captionsize 
Social encounters occur in a variety of environments (a--d), and we propose to study them (and the underlying social relationships) by learning and detecting discrete sets of salient environment-specific interaction categories. 
...
These desiderata motivate us to abandon existing approaches based on coupled hidden Markov models (couples HMMs)~\cite{}\todd{get citations from the ALT proposal}. These approaches are inspired by speech analysis and attempt to model an interaction as an explicit \emph{multi-thread event}~\cite{}\todd{get citations from the ALT proposal} comprised of two or more single-thread events (i.e., two or more temporal sequences of per-person atomic actions) that are interconnected in time. These approaches can theoretically provide very robust detection and recognition of interactions, but they before they can be applied one must optimize for each interaction category both the connectivity of the coupled HMM and its parameters. This is a daunting task that would seem to require hundreds, if not thousands, of labeled training samples for each category.

We propose to develop a new class of representations for social interactions that are better-suited to data-driven analysis, including techniques like domain adaptation, active learning, semi-supervised learning, and unsupervised learning, which are often necessary to reduce the number required training samples to a level that enables widespread practice. The simplest representation of an interaction we will consider is an ensemble of two types of time-varying descriptors: 1) per-agent descriptors that encode the appearance and/or motion of each agent; and 2) relative pairwise descriptors that encode the appearance and/or motion of each agent relative to another. Accordingly, an instance of an interaction spanning $T$ time units and containing $M$ (probably noisy and fragmented) space-time tracks of agents---these could be centroid locations, bounding boxes, silhouettes, etc.---is simply represented by the ensemble $\mathcal{Q}\triangleq \{\mathbf{f}_{m,t},\mathbf{g}_{m,m',t}\}$ containing at most $M\times T$ per-agent descriptors $\mathbf{f}_{m,t}$ and $M\times (M-1)\times T$ per-pair descriptors $\mathbf{g}_{m,m',t}$. Computing the similarity between two interactions amounts to comparing their descriptor-ensembles $\mathcal{Q}$, and detecting an interaction category amounts to searching through space and time for ensembles that are similar to one or more exemplars in that category. Our approach has the important advantage of avoiding the explicit semantic description of social interactions, which is critical when an interaction category cannot easily be broken down according to any pre-defined grammar, or when one lacks the vocabulary to precisely describe the coupled sequences of atomic events that comprise it.

In the remainder of the section we discuss how this representation (and its generalizations) will enable efficient matching and learning.
