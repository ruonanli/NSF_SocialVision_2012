...
These desiderata motivate us to abandon existing approaches based on coupled hidden Markov models (coupled HMMs)~\cite{Brand:CHMM}. These approaches are inspired by speech analysis and attempt to model an interaction as an explicit \emph{multi-thread event}~\cite{Hongeng:act} comprised of two or more single-thread events (i.e., two or more temporal sequences of per-person atomic actions) that are interconnected in time. These approaches can theoretically provide very robust detection and recognition of interactions, but they before they can be applied one must optimize for each interaction category both the connectivity of the coupled HMM and its parameters. This is a daunting task that would seem to require hundreds of labeled training samples for each category.
